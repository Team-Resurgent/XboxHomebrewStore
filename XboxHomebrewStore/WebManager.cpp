...
#include "String.h"
#include "parson.h"
// StoreList::GetActiveUrl() is now read dynamically via
// StoreList::GetActiveUrl() keeping path constants here
...
struct NetworkLockScope {
  NetworkLockScope() {
    if (gNetworkLockInit) {
      EnterCriticalSection(&gNetworkLock);
    }
  }
  ~NetworkLockScope() {
    if (gNetworkLockInit) {
      LeaveCriticalSection(&gNetworkLock);
    }
  }
};
...
static int MultiSocketCallback(CURL *easy, curl_socket_t s, int what,
...
static size_t StringWriteCallback(char *ptr, size_t size, size_t nmemb,
    void *userdata) {
...
static size_t NullWriteCallback(char *ptr, size_t size, size_t nmemb,
    void *userdata) {
  (void)ptr;
  (void)userdata;
  ...
  if (itemsArr) {
    ...
    for (size_t i = 0; i < count; i++) {
      ...
      if (itemObj) {
        AppItem app;
        app.id =
            JsonHelper::ToString(JsonHelper::GetObjectMember(itemObj, "id"));
        app.name =
            JsonHelper::ToString(JsonHelper::GetObjectMember(itemObj, "name"));
        app.author = JsonHelper::ToString(
            JsonHelper::GetObjectMember(itemObj, "author"));
        app.category = JsonHelper::ToString(
            JsonHelper::GetObjectMember(itemObj, "category"));
        app.description = JsonHelper::ToString(
            JsonHelper::GetObjectMember(itemObj, "description"));
        app.latestVersion = JsonHelper::ToString(
            JsonHelper::GetObjectMember(itemObj, "latest_version"));
        app.state =
            JsonHelper::ToBool(JsonHelper::GetObjectMember(itemObj, "state"));
...
static bool ParseCategoriesResponse(const std::string raw,
    CategoriesResponse &out) {
  JSON_Value *root = json_parse_string(raw.c_str());
  if (!root) {
    return false;
  }
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (itemObj) {
      ...
      cat.name =
          JsonHelper::ToString(JsonHelper::GetObjectMember(itemObj, "name"));
      cat.count =
          JsonHelper::ToUInt32(JsonHelper::GetObjectMember(itemObj, "count"));
      out.push_back(cat);
    }
  }
  json_value_free(root);
...
static bool ParseVersionsResponse(const std::string raw,
    VersionsResponse &out) {
  JSON_Value *root = json_parse_string(raw.c_str());
  if (!root) {
    return false;
  }
  ...
  out.author =
      JsonHelper::ToString(JsonHelper::GetObjectMember(rootObj, "author"));
  out.description =
      JsonHelper::ToString(JsonHelper::GetObjectMember(rootObj, "description"));
  out.latestVersion = JsonHelper::ToString(
      JsonHelper::GetObjectMember(rootObj, "latest_version"));
  ...
  if (arr) {
    ...
    for (size_t i = 0; i < count; i++) {
      ...
      if (itemObj) {
        ...
        ver.id =
            JsonHelper::ToString(JsonHelper::GetObjectMember(itemObj, "id"));
        ver.version = JsonHelper::ToString(
            JsonHelper::GetObjectMember(itemObj, "version"));
        ver.size =
            JsonHelper::ToUInt32(JsonHelper::GetObjectMember(itemObj, "size"));
        ver.releaseDate = JsonHelper::ToString(
            JsonHelper::GetObjectMember(itemObj, "release_date"));
        ver.changeLog = JsonHelper::ToString(
            JsonHelper::GetObjectMember(itemObj, "changelog"));
        ver.titleId = JsonHelper::ToString(
            JsonHelper::GetObjectMember(itemObj, "title_id"));
        ver.region = JsonHelper::ToString(
            JsonHelper::GetObjectMember(itemObj, "region"));
        ver.downloadFiles = JsonHelper::ToStringArray(
            JsonHelper::GetObjectMember(itemObj, "download_files"));
        ver.folderName = JsonHelper::ToString(
            JsonHelper::GetObjectMember(itemObj, "folder_name"));
...
static size_t HeaderCaptureCallback(char *ptr, size_t size, size_t nmemb,
    void *userdata) {
  size_t total = size * nmemb;
...
static bool ParseContentDispositionFilename(const std::string &headers,
  ...
  for (;;) {
    ...
    while (fn < headers.size() && (headers[fn] == ' ' || headers[fn] == '\t')) {
      fn++;
    }
...
static bool ExtractGoogleConfirmToken(const std::string &html,
    std::string &token) {
...
static bool ExtractGoogleFilename(const std::string &html,
    std::string &filename) {
  filename.clear();
...
static void RunMultiSocketDownload(CURL *curl, FILE *fp,
      ...
      if (curl_url_set(urlp, CURLUPART_URL, effective_url, 0) == CURLUE_OK) {
        ...
        if (curl_url_get(urlp, CURLUPART_HOST, &host, 0) == CURLUE_OK) {
          ...
          if (hostname == "github.com" ||
              String::EndsWith(hostname, ".githubusercontent.com")) {
  ...
  if (mc != CURLM_OK) {
    ...
    *outRes =
        (mc == CURLM_OUT_OF_MEMORY ? CURLE_OUT_OF_MEMORY : CURLE_SEND_ERROR);
    *outHttpCode = 0;
    return;
  }
  ...
  while (still_running) {
    ...
    if (sockCtx.sock != CURL_SOCKET_BAD) {
      if (sockCtx.what & CURL_POLL_IN) {
        FD_SET(sockCtx.sock, &read_fd);
      }
      if (sockCtx.what & CURL_POLL_OUT) {
        FD_SET(sockCtx.sock, &write_fd);
      }
    }
    ...
    tv.tv_usec =
        (sockCtx.timeout_ms < 0 ? 0 : (sockCtx.timeout_ms % 1000) * 1000);
    ...
    if (r < 0) {
      ...
      if (FD_ISSET(sockCtx.sock, &read_fd)) {
        ev |= CURL_CSELECT_IN;
      }
      if (FD_ISSET(sockCtx.sock, &write_fd)) {
        ev |= CURL_CSELECT_OUT;
      }
...
static bool HandleHtmlResponse(const std::string &url,
    const std::string &filePath,
    std::string *outFinalFileName,
    WebManager::DownloadProgressFn progressFn,
  ...
  if (!WebManager::TryDownloadApiData(confirmedUrl, finalPath, progressFn,
          progressUserData, pCancelRequested,
          nullptr, nullptr)) {
...
bool WebManager::TryDownloadWebData(const std::string url,
    const std::string filePath,
    std::string *outFinalFileName,
  ...
  if (!TryDownloadApiData(url, filePath, progressFn, progressUserData,
          pCancelRequested, &headers, &contentType)) {
  ...
  if (contentType.find("text/html") == std::string::npos) {
    ...
    std::string finalPath = filePath;
    if (ParseContentDispositionFilename(headers, detectedName) &&
        !detectedName.empty()) {
      std::string dir;
      size_t slash = filePath.find_last_of("\\/");
      if (slash != std::string::npos) {
  ...
  return HandleHtmlResponse(url, filePath, outFinalFileName, progressFn,
      progressUserData, pCancelRequested);
}
...
bool WebManager::TryGetApps(AppsResponse &result, int32_t offset, int32_t count,
  ...
  std::string url = StoreList::GetActiveUrl() + store_app_controller +
...
bool WebManager::TryGetVersions(const std::string id,
...
bool WebManager::TryDownloadApiData(const std::string url,
    const std::string filePath,
    DownloadProgressFn progressFn,
    void *progressUserData,
    volatile bool *pCancelRequested,
...
bool WebManager::TryDownloadApp(const std::string id,
    const std::string filePath,
    DownloadProgressFn progressFn,
    void *progressUserData,
    volatile bool *pCancelRequested) {
...
bool WebManager::TryDownloadVersionFile(const std::string versionId,
    int32_t fileIndex,
    const std::string filePath,
    DownloadProgressFn progressFn,
    void *progressUserData,
  ...
  std::string url = StoreList::GetActiveUrl() + "/api/Download/" + versionId +
                    String::Format("?fileIndex=%d", fileIndex);
  return TryDownloadApiData(url, filePath, progressFn, progressUserData,
      pCancelRequested, nullptr, nullptr);
}
...
bool WebManager::TrySyncTime() {
  ...
  sendto(sock, (const char *)packet, sizeof(packet), 0, (sockaddr *)&addr,
      sizeof(addr));
...
bool WebManager::TryGetLatestRelease(std::string &outTag,
  ...
  const char *url = "https://api.github.com/repos/Team-Resurgent/"
                    "XboxHomebrewStore/releases/latest";
  ...
  // Use TryDownloadWebData — it handles the GitHub BearSSL/multi-socket
  // workaround
  ...
  JSON_Value *root = json_parse_string(raw.c_str());
  if (!root) {
    Debug::Print("TryGetLatestRelease: JSON parse failed\n");
    return false;
  }
