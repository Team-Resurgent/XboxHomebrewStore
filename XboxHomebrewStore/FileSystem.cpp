...
FileContainer *getFileContainer(uint32_t fileHandle) {
  std::map<uint32_t, FileContainer>::iterator it =
      mFileContainerMap.find(fileHandle);
...
bool FileSystem::FileGetFileInfoDetail(const std::string path,
    FileInfoDetail &out) {
  ...
  if (fileHandle != INVALID_HANDLE_VALUE) {
    ...
    if (GetFileTime(fileHandle, NULL, &fileTimeAccess, &fileTimeWrite) ==
        FALSE) {
    ...
    if (FileTimeToSystemTime(&fileTimeAccess, &systemTimeAccessLocal) ==
        FALSE) {
      CloseHandle(fileHandle);
      return false;
    }
    ...
    if (fileTimeWrite.dwHighDateTime != 0 || fileTimeWrite.dwLowDateTime) {
      ...
      if (FileTimeToLocalFileTime(&fileTimeWrite, &fileTimeWriteLocal) ==
          FALSE) {
        CloseHandle(fileHandle);
        return false;
      }
      ...
      if (FileTimeToSystemTime(&fileTimeWriteLocal, &systemTimeWriteLocal) ==
          FALSE) {
        CloseHandle(fileHandle);
        return false;
      }
...
std::vector<FileInfoDetail>
FileSystem::FileGetFileInfoDetails(const std::string path) {
  ...
  for (size_t i = 0; i + 1 < fileInfoDetails.size(); i++) {
    for (size_t j = i + 1; j < fileInfoDetails.size(); j++) {
      if (_stricmp(fileInfoDetails[i].path.c_str(),
              fileInfoDetails[j].path.c_str()) > 0) {
...
bool FileSystem::FileOpen(const std::string path, FileMode fileMode,
    uint32_t &fileHandle) {
  const char *access = "";
...
bool FileSystem::FileRead(uint32_t fileHandle, char *readBuffer,
  ...
  bytesRead =
      (uint32_t)fread(readBuffer, 1, bytesToRead, (FILE *)fileContainer->file);
  return true;
}

bool FileSystem::FileWrite(uint32_t fileHandle, char *writeBuffer,
    uint32_t bytesToWrite, uint32_t &bytesWritten) {
  FileContainer *fileContainer = getFileContainer(fileHandle);
  if (fileContainer == NULL) {
    bytesWritten = 0;
    return false;
  }
  bytesWritten = (uint32_t)fwrite(writeBuffer, 1, bytesToWrite,
      (FILE *)fileContainer->file);
  return bytesWritten == bytesToWrite;
}

bool FileSystem::FileWrite(const std::string path, char *writeBuffer,
    uint32_t bytesToWrite, uint32_t &bytesWritten) {
  uint32_t fileHandle;
...
bool FileSystem::FileSeek(uint32_t fileHandle, FileSeekMode fileSeekMode,
    uint32_t offset) {
  int seek = SEEK_CUR;
...
bool FileSystem::SetFileTime(const std::string path, const FileTime fileTime) {
  ...
  HANDLE fileHandle = CreateFileA(path.c_str(), FILE_WRITE_ATTRIBUTES, 0, NULL,
...
bool FileSystem::FileCopy(const std::string sourcePath,
  ...
  while (bytesRead > 0) {
    ...
    if (FileWrite(destFileHandle, buffer, bytesRead, bytesWritten) == false ||
...
bool FileSystem::FileMove(const std::string sourcePath,
    const std::string destPath) {
...
// Tested
std::string FileSystem::CombinePath(const std::string first,
