...
static bool DoUnmount(DriveEntry *ent) {
  ...
  if (ent->kind == DriveKindCdRom && deviceLen > 0 &&
      ent->devicePath[deviceLen - 1] == '\\') {
    deviceLen--;
  }

  STRING sSymlink = {(USHORT)symlink.length(), (USHORT)symlink.length() + 1,
      (PSTR)symlink.c_str()};
  STRING sDevice = {(USHORT)deviceLen, (USHORT)deviceLen + 1,
      (PSTR)ent->devicePath.c_str()};
...
static bool DoMount(DriveEntry *ent) {
  ...
  if (ent->kind == DriveKindCdRom) {
    ...
    if (HalReadSMCTrayState(&trayState, nullptr) == STATUS_SUCCESS &&
        trayState == SMC_TRAY_STATE_MEDIA_DETECT) {
      return true;
    }
    return true;
  }
  ...
  return GetDiskFreeSpaceExA(path.c_str(), nullptr, &totalBytes, nullptr)
             ? true
             : false;
}
...
static bool PathStartsWith(const std::string path, const std::string prefix) {
  ...
  return _strnicmp(path.c_str(), prefix.c_str(), (unsigned int)prefix.size()) ==
         0;
}
...
static bool IsEntryMounted(DriveEntry *ent) {
  ...
  if (ent->kind == DriveKindMemoryUnit) {
    return !ent->devicePath.empty() &&
           InputManager::IsMemoryUnitMounted(ent->devicePath[0]);
  }
  return true;
}
...
std::string DriveMount::MapFtpPath(const std::string path) {
  ...
  for (DriveMap::iterator it = drives.begin(); it != drives.end(); ++it) {
    ...
    if (PathStartsWith(tempPath, mountPointAlias) &&
        (tempPath.size() == mountPointAlias.size() ||
            tempPath[mountPointAlias.size()] == '/' ||
            tempPath[mountPointAlias.size()] == '\\')) {
      DoMount(&ent);
      ...
      if (!localFolder.empty() &&
          (localFolder[0] == '/' || localFolder[0] == '\\')) {
        localFolder = localFolder.substr(1);
      }
      std::string localPath =
          String::Format("%s:\\%s", ent.name.c_str(), localFolder.c_str());
...
bool DriveMount::GetTotalNumberOfBytes(const std::string mountPoint,
    uint64_t &totalSize) {
...
bool DriveMount::GetTotalFreeNumberOfBytes(const std::string mountPoint,
    uint64_t &totalFree) {
  std::string driveName = GetDriveNameFromMountPoint(mountPoint);
  DriveEntry *ent = FindDrive(driveName);
  if (ent == nullptr) {
