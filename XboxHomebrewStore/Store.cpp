...
//    m_nextPageHasNextPage = false;
//    m_nextPageHasPreviousPage = false;
//}
//
////-----------------------------------------------------------------------------
//// Destructor
...
//
////-----------------------------------------------------------------------------
//// LoadAppsPage - Fetch one page of apps from API
////-----------------------------------------------------------------------------
...
//    int visibleCount = ( m_nFilteredCount < STORE_ITEMS_PER_SCREEN ) ? m_nFilteredCount : STORE_ITEMS_PER_SCREEN;
//    m_visibleStart = -1;
//    m_visibleCount = 0;
//    SetVisibleRange( 0, visibleCount );
//    OutputDebugString( String::Format( "Loaded page %d: %d apps (total %d, hasNext=%d)\n", m_nApiPage, m_nItemCount, m_nTotalCount, m_hasNextPage ? 1 : 0 ).c_str() );
//    return TRUE;
//}
//
////-----------------------------------------------------------------------------
//// Next page buffer and navigation
////-----------------------------------------------------------------------------
//void Store::FreeNextPage()
//{
...
//    AppsResponse resp;
//    if( !WebManager::TryGetApps( resp, (uint32_t)( m_nApiPage + 1 ), (uint32_t)STORE_API_PAGE_SIZE, m_currentCategoryFilter, "" ) ) return false;
...
//    {   
//        AppItem* appItem = &resp.items[i];
//        StoreItem* item = &pNew[m_nItemCount + i];
//        item->app = resp.items[i];
//        item->id = 
//        item->category = item->app.category;
//        item->icon = NULL;
...
//    uint32_t state;
//
//    LPDIRECT3DTEXTURE8 icon;
//    LPDIRECT3DTEXTURE8 screenshot;
//
//        if(ImageDownloader::IsCoverCached( item->app.id ) )
//        {
//            std::string path = ImageDownloader::GetCoverCachePath( item->app.id );
...
//        if (item->pIcon == NULL)
//        {
//            item->pIcon = (LPDIRECT3DTEXTURE8)TextureHelper::GetCover();
//            m_imageDownloader.Queue( &item->pIcon, item->app.id, IMAGE_COVER, true );
//        }
//
//        item->nSelectedVersion = 0;
//        item->nVersionScrollOffset = 0;
...
//}
//
//void Store::PreloadNextPage()
//{
//    if( m_pNextItems ) return;
//    if( !m_hasNextPage ) return;
//    AppsResponse resp;
//    if( !WebManager::TryGetApps( resp, (uint32_t)( m_nApiPage + 1 ), (uint32_t)STORE_API_PAGE_SIZE, m_currentCategoryFilter, "" ) )
//        return;
...
//    if( !m_pNextItems ) return;
//    m_nNextItemCount = n;
//    m_nextPageHasNextPage = resp.hasNextPage;
//    m_nextPageHasPreviousPage = resp.hasPreviousPage;
//    for( int i = 0; i < n; i++ )
//    {
...
//    OutputDebugString( String::Format( "Preloaded next page: %d apps (hasNext=%d)\n", n, m_nextPageHasNextPage ? 1 : 0 ).c_str() );
//}
//
//bool Store::GoToNextPage()
//{
...
//    m_nApiPage++;
//    m_nDisplayPage = 0;
//    m_hasNextPage = m_nextPageHasNextPage;
//    m_hasPreviousPage = m_nextPageHasPreviousPage;
//    m_pFilteredIndices = new int[m_nItemCount];
//    if( m_pFilteredIndices )
//    {
//        for( int i = 0; i < m_nItemCount; i++ ) m_pFilteredIndices[i] = i;
...
//{
//    m_imageDownloader.ProcessCompleted();
//}
//
...
////        }
////
////        if(ImageDownloader::IsCoverCached( pItem->app.id ) )
////        {
////            std::string path = ImageDownloader::GetCoverCachePath( pItem->app.id );
////            LPDIRECT3DTEXTURE8 pTex = (LPDIRECT3DTEXTURE8)TextureHelper::LoadFromFile( path );
////            if( pTex != NULL) {
////                pItem->pIcon = pTex;
////            }
...
////            m_imageDownloader.Queue( &pItem->pIcon, pItem->app.id, IMAGE_COVER, true );
////        }
////    }
////}
//
