...
void Drawing::SaveRenderState() {
  ...
  Context::GetD3dDevice()->GetRenderState(D3DRS_ALPHATESTENABLE,
      &savedStatePage[3]);
  ...
  Context::GetD3dDevice()->GetRenderState(D3DRS_STENCILENABLE,
      &savedStatePage[9]);
  Context::GetD3dDevice()->GetRenderState(D3DRS_STENCILFUNC,
      &savedStatePage[10]);
  Context::GetD3dDevice()->GetRenderState(D3DRS_STENCILREF,
      &savedStatePage[11]);
  Context::GetD3dDevice()->GetRenderState(D3DRS_STENCILMASK,
      &savedStatePage[12]);
  Context::GetD3dDevice()->GetRenderState(D3DRS_STENCILWRITEMASK,
      &savedStatePage[13]);
  Context::GetD3dDevice()->GetRenderState(D3DRS_STENCILPASS,
      &savedStatePage[14]);
  Context::GetD3dDevice()->GetRenderState(D3DRS_COLORWRITEENABLE,
      &savedStatePage[15]);
  Context::GetD3dDevice()->GetRenderState(D3DRS_EDGEANTIALIAS,
      &savedStatePage[16]);
  Context::GetD3dDevice()->GetTextureStageState(0, D3DTSS_MINFILTER,
      &savedStatePage[17]);
  Context::GetD3dDevice()->GetTextureStageState(0, D3DTSS_MAGFILTER,
      &savedStatePage[18]);
  Context::GetD3dDevice()->GetTextureStageState(0, D3DTSS_COLOROP,
      &savedStatePage[19]);
  Context::GetD3dDevice()->GetTextureStageState(0, D3DTSS_COLORARG1,
      &savedStatePage[20]);
  Context::GetD3dDevice()->GetTextureStageState(0, D3DTSS_COLORARG2,
      &savedStatePage[21]);
}
...
void Drawing::RestoreRenderState() {
  ...
  Context::GetD3dDevice()->SetRenderState(D3DRS_ALPHABLENDENABLE,
      savedStatePage[0]);
  ...
  Context::GetD3dDevice()->SetRenderState(D3DRS_ALPHATESTENABLE,
      savedStatePage[3]);
  ...
  Context::GetD3dDevice()->SetRenderState(D3DRS_STENCILENABLE,
      savedStatePage[9]);
  Context::GetD3dDevice()->SetRenderState(D3DRS_STENCILFUNC,
      savedStatePage[10]);
  Context::GetD3dDevice()->SetRenderState(D3DRS_STENCILREF, savedStatePage[11]);
  Context::GetD3dDevice()->SetRenderState(D3DRS_STENCILMASK,
      savedStatePage[12]);
  Context::GetD3dDevice()->SetRenderState(D3DRS_STENCILWRITEMASK,
      savedStatePage[13]);
  Context::GetD3dDevice()->SetRenderState(D3DRS_STENCILPASS,
      savedStatePage[14]);
  Context::GetD3dDevice()->SetRenderState(D3DRS_COLORWRITEENABLE,
      savedStatePage[15]);
  Context::GetD3dDevice()->SetRenderState(D3DRS_EDGEANTIALIAS,
      savedStatePage[16]);
  Context::GetD3dDevice()->SetTextureStageState(0, D3DTSS_MINFILTER,
      savedStatePage[17]);
  Context::GetD3dDevice()->SetTextureStageState(0, D3DTSS_MAGFILTER,
      savedStatePage[18]);
  Context::GetD3dDevice()->SetTextureStageState(0, D3DTSS_COLOROP,
      savedStatePage[19]);
  Context::GetD3dDevice()->SetTextureStageState(0, D3DTSS_COLORARG1,
      savedStatePage[20]);
  Context::GetD3dDevice()->SetTextureStageState(0, D3DTSS_COLORARG2,
      savedStatePage[21]);
}

void Drawing::Swizzle(const void *src, const uint32_t &depth,
...
bool Drawing::TryCreateImage(uint8_t *imageData, D3DFORMAT format,
    int32_t width, int32_t height, Image *image) {
  image->width = width;
  image->height = height;

  if (FAILED(D3DXCreateTexture(Context::GetD3dDevice(), image->width,
          image->height, 1, 0, format, D3DPOOL_DEFAULT,
          &image->texture))) {
    return false;
  }
  ...
  if (SUCCEEDED(image->texture->LockRect(0, &lockedRect, nullptr, 0))) {
    ...
    Swizzle(tempBuffer, 4, surfaceDesc.Width, surfaceDesc.Height,
        lockedRect.pBits);
    free(tempBuffer);
...
bool Drawing::TryGenerateBitmapFont(void *context, const std::string fontName,
    int32_t fontStyle, int32_t fontSize,
    int32_t lineHeight, int32_t spacing,
    int32_t textureDimension,
    BitmapFont *bitmapFont) {
  ssfn_select((ssfn_t *)context, SSFN_FAMILY_ANY, fontName.c_str(), fontStyle,
      fontSize);
  ...
  while (*currentCharPos) {
    ...
    int bounds_left;
    int bounds_top;
    int ret = ssfn_bbox((ssfn_t *)context, currentChar, &bounds_width,
        &bounds_height, &bounds_left, &bounds_top);
    if (ret != 0) {
      free(currentChar);
      continue;
    }
  ...
  bool result = TryCreateImage((uint8_t *)imageData, D3DFMT_A8R8G8B8,
      textureWidth, textureHeight, &bitmapFont->image);
  free(imageData);
  return result;
}

void Drawing::DrawFont(BitmapFont *font, const std::string message,
  ...
  while (*p) {
    ...
    if (vertexCount + 6 > (UINT)DRAW_BATCH_MAX_VERTS) {
      Context::GetD3dDevice()->DrawPrimitiveUP(
          D3DPT_TRIANGLELIST, vertexCount / 3, batchBuf, sizeof(TEXVERTEX));
      vertexCount = 0;
    }
...
void Drawing::DrawFontWrapped(BitmapFont *font, const std::string message,
    uint32_t color, float x, float y,
    float maxWidth) {
  const float sx = Context::GetScaleX(), sy = Context::GetScaleY();
  x *= sx;
  ...
  while (*p) {
    ...
    const Rect &rect = it->second;
    float charWidth =
        ((float)rect.width + (lineWidth > 0 ? (float)font->spacing : 0)) * sx;

    if (unicode == ' ') {
      const char *nextWordStart = p;
      float nextWordW =
          MeasureWordWidth(font, nextWordStart, &nextWordStart) * sx;
      if (lineWidth > 0 && lineWidth + (float)font->spacing * sx +
                                   (float)rect.width * sx + nextWordW >
                               maxWidth) {
        xPos = x;
        yPos += (float)font->line_height * sy;
        lineWidth = 0;
...
void Drawing::DrawFilledRect(uint32_t color, float x, float y, float width,
    float height) {
  const float sx = Context::GetScaleX(), sy = Context::GetScaleY();
  x *= sx;
  y *= sy;
  ...
  Context::GetD3dDevice()->SetVertexShader(D3DFVF_XYZRHW | D3DFVF_DIFFUSE);
  Context::GetD3dDevice()->SetTexture(0, nullptr);
  Context::GetD3dDevice()->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, vertices,
      sizeof(VERTEX));
  RestoreRenderState();
}

void Drawing::DrawTexturedRect(D3DTexture *texture, uint32_t diffuse, float x,
  ...
  Context::GetD3dDevice()->SetTextureStageState(0, D3DTSS_COLOROP,
      D3DTOP_MODULATE);
  Context::GetD3dDevice()->SetTextureStageState(0, D3DTSS_ALPHAOP,
      D3DTOP_MODULATE);
  Context::GetD3dDevice()->SetTextureStageState(0, D3DTSS_COLORARG1,
      D3DTA_TEXTURE);
  Context::GetD3dDevice()->SetTextureStageState(0, D3DTSS_COLORARG2,
...
void Drawing::DrawNinePatch(D3DTexture *texture, uint32_t diffuse, float x,
    float y, float width, float height,
    float cornerWidthPx, float cornerHeightPx,
  ...
  const float contentW =
      contentWidthPx > 0 ? contentWidthPx : (float)(int32_t)surfaceW;
  const float contentH =
      contentHeightPx > 0 ? contentHeightPx : (float)(int32_t)surfaceH;
  ...
  Context::GetD3dDevice()->SetTextureStageState(0, D3DTSS_COLORARG2,
      D3DTA_DIFFUSE);
  Context::GetD3dDevice()->DrawPrimitiveUP(D3DPT_TRIANGLELIST, 18, batchBuf,
      sizeof(TEXVERTEX));
  RestoreRenderState();
}
...
void Drawing::BeginStencil(float x, float y, float w, float h) {
  ...
  Context::GetD3dDevice()->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, quad,
      sizeof(stencil_vertex));

  Context::GetD3dDevice()->SetRenderState(
