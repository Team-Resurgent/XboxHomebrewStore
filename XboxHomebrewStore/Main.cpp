...
DISPLAY_MODE displayModes[] = {
    ...
    //{  1920,   1080,    false,  true,  60 },         // 1920x1080 interlaced
    // 16x9
...
bool SupportsMode(DISPLAY_MODE mode, DWORD dwVideoStandard, DWORD dwVideoFlags) {
  ...
  if (mode.dwHeight == 480 && mode.fWideScreen &&
      !(dwVideoFlags & XC_VIDEO_FLAGS_WIDESCREEN)) {
    return false;
  }

  if (mode.dwHeight == 480 && mode.fProgressive &&
...
bool InitD3D() {
  ...
  params.Flags = displayModes[currentMode].fProgressive
                     ? D3DPRESENTFLAG_PROGRESSIVE
                     : D3DPRESENTFLAG_INTERLACED;
  params.Flags |=
      displayModes[currentMode].fWideScreen ? D3DPRESENTFLAG_WIDESCREEN : 0;
...
//-----------------------------------------------------------------------------
VOID Render() {
  // Clear the backbuffer to dark gray (D3DCLEAR_STENCIL set for 32-bit depth
  // perf)
  g_pd3dDevice->Clear(0, nullptr, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER | D3DCLEAR_STENCIL, D3DCOLOR_XRGB(33, 33, 33), 1.0f, 0);
