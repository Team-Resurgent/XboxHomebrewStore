...
// File format: 4-byte count, 4-byte activeIndex, then count * sizeof(StoreEntry)
...
bool StoreList::Save()
{
    ...
    bool ok = FileSystem::FileWrite(fileHandle, (char*)header,
                                    STORE_LIST_HEADER_SIZE, bytesWritten);
...
{
    if (!mLoaded) Load();
    if (index < 0 || index >= mCount) return nullptr;
...
{
    if (!mLoaded) Load();
    if (mCount == 0) return DEFAULT_STORE_URL;
...
{
    if (!mLoaded) Load();
    if (mCount >= STORE_LIST_MAX) return -1;
...
bool StoreList::Delete(int32_t index)
{
    if (!mLoaded) Load();
    if (index < 0 || index >= mCount) return false;
    if (mCount == 1) return false;  // always keep at least one
...
bool StoreList::WasStoreChanged()
{
    return mStoreChanged;
}

void StoreList::ClearStoreChangedFlag()
{
    mStoreChanged = false;
}