...
int incrementConnections() { return _InterlockedIncrement(&activeConnections); }

void decrementConnections() { _InterlockedDecrement(&activeConnections); }
...
std::string resolveRelative(const std::string currentVirtual,
    const std::string relativeVirtual) {
...
std::vector<FileInfoDetail> getDirectoryListing(const std::string virtualPath) {
  ...
  std::vector<FileInfoDetail> fileInfoDetails =
      FileSystem::FileGetFileInfoDetails(ftpPath);
...
bool WINAPI FtpServer::ConnectionThread(uint64_t sCmd) {
  ...
  szPeerName =
      String::Format("%u.%u.%u.%u", saiCmdPeer.sin_addr.S_un.S_un_b.s_b1,
  ...
  SocketSendString(sCmd,
      ...
      "Proceed with login.\r\n",
      SERVERID, szPeerName.c_str(), ntohs(saiCmdPeer.sin_port));
  ...
  while (mStopRequested == false) {
...

    {
      ...
      while (line.size() > 0 &&
             (line[line.size() - 1] == ' ' || line[line.size() - 1] == '\r' ||
                 line[line.size() - 1] == '\n' ||
      ...
      while (pszParam.size() > 0 &&
             (pszParam[0] == ' ' || pszParam[0] == '\r' ||
                 pszParam[0] == '\n' || (unsigned char)pszParam[0] < 32)) {
        pszParam = pszParam.substr(1);
      }
      while (pszParam.size() > 0 &&
             (pszParam[pszParam.size() - 1] == ' ' ||
                 pszParam[pszParam.size() - 1] == '\r' ||
                 pszParam[pszParam.size() - 1] == '\n' ||
    ...
    if (String::EqualsIgnoreCase(szCmd, "USER")) {
      if (pszParam.empty()) {
        ...
        SocketSendString(
            sCmd, "503 Already logged in. Use REIN to change users.\r\n");
        continue;
      } else {
        user = pszParam;
    ...
    if (String::EqualsIgnoreCase(szCmd, "PASS")) {
      if (user.empty()) {
        ...
        if (String::EqualsIgnoreCase(user, "xbox") &&
            String::EqualsIgnoreCase(pszParam, "xbox")) {
          if (incrementConnections() <= mMaxConnections) {
            ...
            SocketSendString(sCmd, "421 Your login was refused due to a server "
                                   "connection limit.\r\n");
            break;
          }
        } else {
    ...
    // }
    ...
    else if (String::EqualsIgnoreCase(szCmd, "PWD") ||
             String::EqualsIgnoreCase(szCmd, "XPWD")) {
      if (!isLoggedIn) {
    ...
    else if (String::EqualsIgnoreCase(szCmd, "CWD") ||
             String::EqualsIgnoreCase(szCmd, "XCWD")) {
      if (pszParam.empty()) {
        SocketSendString(sCmd,
            "501 Syntax error in parameters or arguments.\r\n");
      } else if (!isLoggedIn) {
        ...
        if (isFolder == true) {
          ...
          SocketSendString(sCmd,
              "550 %s failed. \"%s\": directory not found.\r\n",
              szCmd.c_str(), newVirtual.c_str());
        }
      }
    }

    else if (String::EqualsIgnoreCase(szCmd, "CDUP") ||
             String::EqualsIgnoreCase(szCmd, "XCUP")) {
      if (!isLoggedIn) {
        SocketSendString(sCmd, "530 Not logged in.\r\n");
      } else {
      ...
      if (pszParam.empty() ||
          (!(dw = atoi(pszParam.c_str())) && (pszParam[0] != '0'))) {
        SocketSendString(sCmd,
            "501 Syntax error in parameters or arguments.\r\n");
      } else if (!isLoggedIn) {
        SocketSendString(sCmd, "530 Not logged in.\r\n");
      ...
      if (pszParam.empty()) {
        ...
        dw = sscanf(pszParam.c_str(), "%hu,%hu,%hu,%hu,%hu,%hu", &h1, &h2, &h3,
            &h4, &p1, &p2);
      ...
      if (!isLoggedIn) {
        ...
        while (!found) {
          ...
          if (SocketUtility::CreateSocket(AF_INET, SOCK_STREAM, IPPROTO_TCP,
                  sPasv)) {
            found = true;
          } else {
            Debug::Print("Error: Could not create socket on port %u, trying to "
                         "find next available.\n",
                portTest);
          }
          portTest++;
        ...
        SocketSendString(sCmd,
            ...
            addr.ina.S_un.S_un_b.s_b1, addr.ina.S_un.S_un_b.s_b2,
            addr.ina.S_un.S_un_b.s_b3, addr.ina.S_un.S_un_b.s_b4,
            ((unsigned char *)&saiPasv.sin_port)[0],
            ((unsigned char *)&saiPasv.sin_port)[1]);
      }
    }

    else if (String::EqualsIgnoreCase(szCmd, "LIST") ||
             String::EqualsIgnoreCase(szCmd, "NLST")) {
      if (!isLoggedIn) {
        SocketSendString(sCmd, "530 Not logged in.\r\n");
      } else {
        ...
        std::string newVirtual = pathArg.empty()
                                     ? currentVirtual
                                     : resolveRelative(currentVirtual, pathArg);

        std::vector<FileInfoDetail> fileInfoDetails =
            getDirectoryListing(newVirtual);
        if (!fileInfoDetails.empty()) {
          sData = EstablishDataConnection(&saiData, &sPasv);
          if (sData) {
            SocketSendString(
                sCmd, "150 Opened %s mode data connection for \"%s\".\r\n",
            ...
            for (size_t i = 0; i < fileInfoDetails.size(); i++) {
              ...
              if (String::EqualsIgnoreCase(szCmd, "NLST")) {
                ...
                sprintf(szLine, "%c--------- 1 ftp ftp %llu ",
                    fileInfoDetail.isDirectory ? 'd' : '-',
                    fileInfoDetail.size);
                strcat(szLine, months[fileInfoDetail.writeTime.month - 1]);
                sprintf(szLine + strlen(szLine), " %2i %.2i:%.2i ",
                    fileInfoDetail.writeTime.day,
                    fileInfoDetail.writeTime.hour,
                    fileInfoDetail.writeTime.minute);
            ...
            SocketSendString(sCmd, "226 %s command successful.\r\n",
                String::EqualsIgnoreCase(szCmd, "NLST") ? "LIST"
                                                        : "NLST");
          } else {
      ...
      if (!isLoggedIn) {
        ...
        if (!fileInfoDetails.empty()) {
          ...
          for (size_t i = 0; i < fileInfoDetails.size(); i++) {
            ...
            strcat(szLine, months[fileInfoDetail.writeTime.month - 1]);
            sprintf(szLine + strlen(szLine), " %2i %.2i:%.2i ",
                fileInfoDetail.writeTime.day, fileInfoDetail.writeTime.hour,
                fileInfoDetail.writeTime.minute);
            strcat(szLine, fileInfoDetail.path.c_str());
    ...
    else if (String::EqualsIgnoreCase(szCmd, "STOR") ||
             String::EqualsIgnoreCase(szCmd, "APPE")) {
      if (pszParam.empty()) {
        SocketSendString(sCmd,
            "501 Syntax error in parameters or arguments.\r\n");
      } else if (!isLoggedIn) {
        ...
        uint32_t fileHandle;
        if (FileSystem::FileOpen(ftpPath, FileModeWriteUpdate, fileHandle) ==
            true) {
      ...
      if (pszParam.empty()) {
        ...
        if (dw == 1) {
          ...
          if (FileSystem::FileGetFileInfoDetail(ftpPath, fileInfoDetail)) {
            SocketSendString(
                sCmd, "213 %04u%02u%02u%02u%02u%02u\r\n",
                fileInfoDetail.writeTime.year, fileInfoDetail.writeTime.month,
                fileInfoDetail.writeTime.day, fileInfoDetail.writeTime.hour,
                fileInfoDetail.writeTime.minute,
      ...
      if (pszParam.empty()) {
        ...
        std::string ftpPath = DriveMount::MapFtpPath(newVirtual);
        bool exists;
        if ((FileSystem::FileExists(ftpPath, exists) == true &&
                exists == true) ||
            (FileSystem::DirectoryExists(ftpPath, exists) == true &&
                exists == true)) {
          rnfr = ftpPath;
          SocketSendString(
              sCmd, "350 \"%s\": File/Directory exists; proceed with RNTO.\r\n",
              newVirtual.c_str());
        } else {
      ...
      if (pszParam.empty()) {
        ...
        SocketSendString(sCmd,
            "503 Bad sequence of commands. Send RNFR first.\r\n");
      } else {
        std::string newVirtual = resolveRelative(currentVirtual, pszParam);
        std::string ftpPath = DriveMount::MapFtpPath(newVirtual);
    ...
    else if (String::EqualsIgnoreCase(szCmd, "MKD") ||
             String::EqualsIgnoreCase(szCmd, "XMKD")) {
      if (pszParam.empty()) {
        SocketSendString(sCmd,
            "501 Syntax error in parameters or arguments.\r\n");
      } else if (!isLoggedIn) {
    ...
    else if (String::EqualsIgnoreCase(szCmd, "RMD") ||
             String::EqualsIgnoreCase(szCmd, "XRMD")) {
      if (pszParam.empty()) {
        SocketSendString(sCmd,
            "501 Syntax error in parameters or arguments.\r\n");
      } else if (!isLoggedIn) {
  ...
  while (mStopRequested == false) {
    ...
    if (result == 1 && SocketUtility::AcceptSocket(sListen, sIncoming)) {
      HANDLE connectionThreadHandle =
          CreateThread(0, 0, (LPTHREAD_START_ROUTINE)ConnectionThread,
              (void *)sIncoming, 0, NULL);
...
bool FtpServer::Init() {
  ...
  mListenThreadHandle =
      CreateThread(0, 0, (LPTHREAD_START_ROUTINE)ListenThread, 0, 0, NULL);
...
FtpServer::ReceiveStatus
FtpServer::SocketReceiveString(uint64_t s, char *psz, uint32_t dwMaxChars,
    uint32_t *pdwCharsReceived) {
  uint32_t dwChars = 0;
  ...
  for (;;) {
    ...
    if (status == ReceiveStatus_OK) {
      ...
      psz += dw;
               ...
               status == ReceiveStatus_Insufficient_Buffer) {
      statusError = status;
      break;
    } else {
      return status;
...
FtpServer::ReceiveStatus
FtpServer::SocketReceiveLetter(uint64_t s, char *pch, uint32_t dwMaxChars,
    uint32_t *pdwCharsReceived) {
  char buf[4];
  uint32_t dw;
  TIMEVAL tv;
...
FtpServer::ReceiveStatus FtpServer::SocketReceiveData(uint64_t s, char *psz,
    uint32_t dwBytesToRead,
    uint32_t *pdwBytesRead) {
  uint32_t dw;
  TIMEVAL tv;
  fd_set fds;
...
uint64_t FtpServer::EstablishDataConnection(sockaddr_in *psaiData,
    uint64_t *psPasv) {
  uint64_t sData;
  uint32_t dw;
  TIMEVAL tv;
...
bool FtpServer::ReceiveSocketFile(uint64_t sCmd, uint64_t sData,
    uint32_t fileHandle) {
  ...
  while (true) {
    while (!fileComplete &&
           totalBytesToWrite < (uint32_t)driveSectorBufferSize) {
      uint32_t bytesRead = 0;
      if (SocketReceiveData(sData, combinedBuffer + totalBytesToWrite,
              socketBufferSize, &bytesRead) != ReceiveStatus_OK) {
        free(combinedBuffer);
        return false;
      }
    ...
    uint32_t bytesToWrite =
        min((uint32_t)driveSectorBufferSize, totalBytesToWrite);
    if (!FileSystem::FileWrite(fileHandle, combinedBuffer, bytesToWrite,
            bytesWritten)) {
...
bool FtpServer::SendSocketFile(uint64_t sCmd, uint64_t sData,
    uint32_t fileHandle, uint32_t *pdwAbortFlag) {
  ...
  while (true) {
    uint32_t bytesRead;
    if (FileSystem::FileRead(fileHandle, szBuffer, bufferSize, bytesRead) ==
        false) {
    ...
    while (bytesToSend > 0) {
      ...
      if (queueLength > 0) {
        ...
        if (SocketReceiveString(sCmd, cmdBuf, 512, &bytes_received) ==
            ReceiveStatus_OK) {
          ...
          if (String::EqualsIgnoreCase(cmdLine, "ABOR")) {
            *pdwAbortFlag = 1;
            ...
            SocketSendString(
                sCmd, "500 Only command allowed at this time is ABOR.\r\n");
          }
        }
      }
      bytesToSend -= sent;
