...
{
    return (p <= IP_TRIG_DEADZONE) ? IP_TRIG_INIT_MAX_MS
        : IP_MapInvQuad(IP_TRIG_INIT_MIN_MS, IP_TRIG_INIT_MAX_MS, p);
}
static DWORD IP_TrigInterval(BYTE p)
{
    return (p <= IP_TRIG_DEADZONE) ? IP_TRIG_INTV_SLOW_MS
...
// ==========================================================================
InstallPathScene::InstallPathScene(InstallPathConfirmedCb callback, void* userData,
                                   const char* title)
    : mCallback(callback)
    , mUserData(userData)
        ...
        if (!selectName.empty())
        {
            for (int32_t i = 0; i < (int32_t)mItems.size(); i++)
            {
                if (String::EqualsIgnoreCase(
                        FileSystem::GetFileName(mItems[i].path), selectName))
                {
                    mSelectedIndex = i;
                    break;
                }
...
{
    if (index < 0 || index >= (int32_t)mItems.size()) return "";
...
int32_t InstallPathScene::JumpToNextLetter(int32_t start, int dir) const
{
    ...
    for (int32_t s = 0; s < n; s++)
    {
        ...
        if (idx >= n) idx = 0;
        if (idx < 0)  idx = n - 1;
        if (GetKeyChar(GetDisplayName(idx)) != curKey) return idx;
    }
    return start;
}
...
void InstallPathScene::HandleKeyboardResult()
{
    ...
    if (name.empty()) { mKbPurpose = KbNone; return; }
...
{
    if (mItems.empty() || mCurrentPath.empty()) return;
...
void InstallPathScene::Update()
{
    ...
    if (mKeyboard.IsOpen())
    {
        ...
        // IsCancelled() — keyboard closed itself, nothing to do
        return;
    }
    ...
    if (mDeleteOpen)
    {
        if (InputManager::ControllerPressed(ControllerA, -1)) { DeleteConfirm(); return; }
        if (InputManager::ControllerPressed(ControllerB, -1)) { mDeleteOpen = false; return; }
        return;
    }
    ...
    if (InputManager::ControllerPressed(ControllerY, -1))
    {
        ...
        {
            mKbPurpose = KbCreate;
            mKeyboard.Open("New Folder Name", "", 64,
                           KbFilterFilename, COLOR_FOCUS_HIGHLIGHT);
        }
        return;
    }
    // White – rename selected folder
...
void InstallPathScene::Render()
{
    Drawing::DrawTexturedRect(
        TextureHelper::GetBackground(), 0xFFFFFFFF,
    ...
    if (mDeleteOpen)        RenderDeleteConfirm();
    if (mKeyboard.IsOpen()) mKeyboard.Render();
}
                                          ...
                                          const char* iconName, const char* label)
{
    ...
    if (icon != nullptr)
    {
        Drawing::DrawTexturedRect(icon, 0xffffffff, x, footerY + 10.0f, iconW, iconH);
        x += iconW + 4.0f;
    }
...
void InstallPathScene::RenderFooter()
{
    ...
    Drawing::DrawTexturedRect(
        TextureHelper::GetFooter(), 0xffffffff,
...
void InstallPathScene::RenderList()
{
    ...
    int32_t start = 0;
    if (count > maxVisible)
        start = Math::ClampInt32(mSelectedIndex - maxVisible / 2,
                                 0, count - maxVisible);
    ...
    for (int32_t i = 0; i < visibleCount; i++)
    {
        ...
        if (sel)
            Drawing::DrawFilledRect(COLOR_FOCUS_HIGHLIGHT, 0.0f, ry, screenW, rowH - 2.0f);
        else if (i % 2 == 0)
    ...
    std::string counter = String::Format("Item %d of %d", mSelectedIndex + 1, count);
    float cw = 0.0f;
