...
{
    if (mStoreVersions.versions.empty()) return;

    mWaitingForInstallPath = true;
    mPendingInstallPath    = "";

    InstallPathScene* pathScene = new InstallPathScene(OnInstallPathConfirmed, this,
                                                        "Select Install Path");
    ...
    for (size_t f = 0; downloadSucceeded && f < ver->downloadFiles.size(); f++)
    {
        ...
        if (isUrl)
        {
            ...
            if (slash != std::string::npos && slash + 1 < entry.size())
            {
                ...
                size_t q = temp.find('?');
                if (q != std::string::npos) temp = temp.substr(0, q);
                if (temp.empty()) temp = "download";
                fileName = temp;
            }
            ...
            downloadSucceeded = WebManager::TryDownloadWebData(
                entry, filePath, &actualName,
                DownloadProgressCb, scene,
        ...
        for (size_t f = 0; unpackSucceeded && f < downloadedFileNames.size(); f++)
        {
            ...
            if (EndsWithZip(localName))
            {
                unpackSucceeded = xunzipFromFile(
                    srcPath.c_str(), installPath.c_str(),
                    true, true, false,
                    UnpackProgressCb, scene);
            }
            else
            {
...
void VersionScene::Render()
{
    ...
    Drawing::DrawTexturedRect(TextureHelper::GetBackground(), 0xFFFFFFFF,
        0.0f, 0.0f, (float)Context::GetScreenWidth(), (float)Context::GetScreenHeight());
...
{
    float sidebarHeight = (float)(Context::GetScreenHeight() - ASSET_SIDEBAR_Y) - ASSET_FOOTER_HEIGHT;
    Drawing::DrawTexturedRect(TextureHelper::GetSidebar(), 0xffffffff,
        0, ASSET_SIDEBAR_Y, ASSET_SIDEBAR_WIDTH, sidebarHeight);
    ...
    if (versionCount >= maxItems)
        start = Math::ClampInt32(mHighlightedVersionIndex - (maxItems / 2),
                                 0, versionCount - maxItems);
    ...
    for (int32_t pass = 0; pass < 3; pass++)
    {
        ...
        if (pass == 1)
            Drawing::BeginStencil(16, ASSET_SIDEBAR_Y, ASSET_SIDEBAR_WIDTH - 32.0f, sidebarHeight);
        ...
        for (int32_t i = 0; i < itemCount; i++)
        {
            ...
            if (pass == 0)
            {
                if (highlighted || focused)
                    Drawing::DrawTexturedRect(TextureHelper::GetCategoryHighlight(),
                        focused ? COLOR_FOCUS_HIGHLIGHT : COLOR_HIGHLIGHT,
                        0.0f, y - 32.0f,
            ...
            {
                if (focused)
                    Font::DrawTextScrolling(FONT_NORMAL, storeVersion->version, COLOR_WHITE,
                        16.0f, y, ASSET_SIDEBAR_WIDTH - 64.0f, storeVersion->versionScrollState);
                else
                {
                ...
                else if (storeVersion->state == 2)
                {
                    if (!focused)
                        Drawing::DrawTexturedRect(TextureHelper::GetCategoryHighlight(),
                            0x40ffffff, 0.0f, y - 32.0f,
                            ASSET_SIDEBAR_HIGHLIGHT_WIDTH, ASSET_SIDEBAR_HIGHLIGHT_HEIGHT);
                    Drawing::DrawTexturedRect(TextureHelper::GetUpdateBadge(), 0xFFFFFFFF,
...
void VersionScene::RenderListView()
{
    ...
    mListViewContentHeight =
        (float)ASSET_SCREENSHOT_HEIGHT + 16.0f + 30.0f + 30.0f
        + mDescriptionHeight + 8.0f
    ...
    if (mListViewScrollOffset > maxScroll) mListViewScrollOffset = maxScroll;
    ...
    Drawing::BeginStencil(ASSET_SIDEBAR_WIDTH, ASSET_SIDEBAR_Y,
        (float)Context::GetScreenWidth() - ASSET_SIDEBAR_WIDTH, listVisibleHeight);
    ...
    if (screenshot == nullptr)
    {
        ...
        else
        {
            ...
            mImageDownloader->Queue(&mStoreVersions.screenshot,
                mStoreVersions.appId, IMAGE_SCREENSHOT);
        }
    }
    Drawing::DrawTexturedRect(screenshot, 0xFFFFFFFF,
        titleXPos, gridY, ASSET_SCREENSHOT_WIDTH, ASSET_SCREENSHOT_HEIGHT);
    ...
    if (cover == nullptr)
    {
        ...
        else
        {
            ...
            mImageDownloader->Queue(&mStoreVersions.cover,
                mStoreVersions.appId, IMAGE_COVER);
        }
    }
    Drawing::DrawTexturedRect(cover, 0xFFFFFFFF,
        216 + ASSET_SCREENSHOT_WIDTH, gridY, 144, 204);
    ...
    Font::DrawTextWrapped(FONT_NORMAL, mStoreVersions.description, COLOR_TEXT_GRAY,
    ...
    Font::DrawText(FONT_NORMAL, String::FormatSize(storeVersion->size), COLOR_TEXT_GRAY,
                   infoXPos, gridY);
...
void VersionScene::RenderAfterInstallDialog()
{
    ...
    Font::DrawText(FONT_NORMAL, "Install complete!", COLOR_WHITE,
                   px + 16.0f, py + 16.0f);
...
void VersionScene::Update()
{
    ...
    if (listMaxScroll < 0.0f) listMaxScroll = 0.0f;

    if (mSideBarFocused)
    {
        if (InputManager::ControllerPressed(ControllerDpadUp, -1))
        {
            mHighlightedVersionIndex = mHighlightedVersionIndex > 0
        ...
        {
            mHighlightedVersionIndex = mHighlightedVersionIndex < (int32_t)mStoreVersions.versions.size() - 1
                ? mHighlightedVersionIndex + 1
                : 0;
        }
