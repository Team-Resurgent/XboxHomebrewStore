...
static void UpdateDownloadProgressCb(uint32_t dlNow, uint32_t dlTotal, void* userData)
{
...
static DWORD WINAPI UpdateThreadProc(LPVOID param)
{
    ...
    bool ok = WebManager::TryDownloadWebData(
        scene->mUpdateZipUrl, zipPath, &actualName,
        UpdateDownloadProgressCb, scene,
        (volatile bool*)&scene->mUpdateCancelRequested);
    ...
    {
        HANDLE hTest = CreateFileA(zipPath.c_str(), GENERIC_READ, FILE_SHARE_READ,
...
void LoadingScene::Update()
{
    ...
    if (mUpdateStatus == UPDATE_DOWNLOADING || mUpdateStatus == UPDATE_UNPACKING)
        return;
    ...
    if (mUpdateStatus == UPDATE_REBOOTING)
    {
        ...
        // XLaunchNewImage uses D:\ not HDD0-D:\, and needs a valid LAUNCH_DATA struct
...
void LoadingScene::Render()
{
    ...
    if (mUpdateAvailable && mUpdatePromptShown && mUpdateStatus == UPDATE_IDLE)
    {
        ...
        std::string title = String::Format("Update available: %s", mUpdateTag.c_str());
        ...
        Font::DrawText(FONT_NORMAL, sub,   COLOR_TEXT_GRAY, cx - sw  * 0.5f, py + 46.0f);
        Font::DrawText(FONT_NORMAL, sub2,  COLOR_TEXT_GRAY, cx - s2w * 0.5f, py + 70.0f);
    ...
    if (mUpdateStatus == UPDATE_DOWNLOADING || mUpdateStatus == UPDATE_UNPACKING)
    {
        ...
        if (mUpdateStatus == UPDATE_DOWNLOADING && mUpdateDlTotal > 0)
        {
            ...
            std::string prog = String::Format("%s / %s",
                String::FormatSize(mUpdateDlNow).c_str(),
                String::FormatSize(mUpdateDlTotal).c_str());
            float pw = 0.0f;
            Font::MeasureText(FONT_NORMAL, prog, &pw);
            Font::DrawText(FONT_NORMAL, prog, COLOR_TEXT_GRAY, cx - pw * 0.5f, barY + barH + 8.0f);
        }
        else
        {
    ...
    Font::DrawText(FONT_LARGE, "Loading...",  COLOR_WHITE, cx - loadingW  * 0.5f, cy - 40.0f);
    Font::DrawText(FONT_LARGE, progressText,  COLOR_WHITE, cx - progressW * 0.5f, cy);
}