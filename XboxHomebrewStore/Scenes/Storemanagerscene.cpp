...
{
    if (StoreList::GetCount() <= 1) return;
    mDeleteIndex = idx;
    mDeleteOpen  = true;
}
...
{
    if (StoreList::GetCount() >= STORE_LIST_MAX) return;
    mOskIsEdit  = false;
    mOskEditIndex = -1;
    OskOpen(OskName, "");
}
...
void StoreManagerScene::OskConfirmed(const std::string& value)
{
    if (mOskStep == OskName)
    {
        ...
        if (mOskIsEdit)
        {
            ...
            if (e != nullptr) urlPrefill = std::string(e->url);
        }
...
void StoreManagerScene::Update()
{
    ...
    // How many rows fit between LIST_TOP and the footer
    // ROW_H=76, gap=3 => each row takes 79px. LIST_TOP~68, footer~480 => ~5 rows visible
    ...
    if (InputManager::ControllerPressed(ControllerDpadUp, -1))
    {
        ...
        // Scroll up if selected moved above visible area
        if (mSelectedRow < mScrollOffset)
            mScrollOffset = mSelectedRow;
        // Wrap-around: jumped to bottom, scroll to show it
...
void StoreManagerScene::RenderList()
{
    ...
    // Info line: "X of Y stores configured"
    std::string info = String::Format("%d store%s configured",
        count, count == 1 ? "" : "s");
    Font::DrawText(FONT_NORMAL, info.c_str(), COLOR_TEXT_GRAY,
        rowX, LIST_TOP - 2.0f);
    ...
    for (int32_t i = 0; i < count; i++)
    {
        ...
        if (rowY + ROW_H > footerY) break;  // clipped by footer
...
// --------------------------------------------------------------------------
void StoreManagerScene::RenderStoreRow(float x, float y, float w,
                                        bool selected, int32_t idx)
{
    StoreEntry* e = StoreList::GetEntry(idx);
    if (e == nullptr) return;
    ...
    // Background
    uint32_t bg = selected ? 0xFF363636
                : isActive ? 0xFF252535
                :             0xFF222222;
    ...
    uint32_t circleCol = isActive ? 0xFF66BB6A : (selected ? COLOR_FOCUS_HIGHLIGHT : 0xFF444444);
    ...
    std::string displayName = Font::TruncateText(FONT_NORMAL, std::string(e->name), nameMaxW);
    ...
    std::string displayUrl = Font::TruncateText(FONT_NORMAL, std::string(e->url), urlMaxW);
    Font::DrawText(FONT_NORMAL, displayUrl.c_str(), COLOR_TEXT_GRAY, tx, y + 34.0f);
    ...
    if (isActive)
    {
        ...
        Drawing::DrawFilledRect(0xFF1B3A1B, bx - 8.0f, y + 22.0f, bw + 16.0f, 24.0f);
