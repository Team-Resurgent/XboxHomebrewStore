...
class FileSystem {
public:
  static bool FileGetFileInfoDetail(const std::string path,
      FileInfoDetail &out);
  static std::vector<FileInfoDetail>
  FileGetFileInfoDetails(const std::string path);

  static bool FileOpen(const std::string path, FileMode fileMode,
      uint32_t &fileHandle);
  static bool FileRead(uint32_t fileHandle, char *readBuffer,
      uint32_t bytesToRead, uint32_t &bytesRead);
  static bool FileWrite(uint32_t fileHandle, char *writeBuffer,
      uint32_t bytesToWrite, uint32_t &bytesWritten);
  static bool FileWrite(const std::string path, char *writeBuffer,
      uint32_t bytesToWrite, uint32_t &bytesWritten);
  static bool FileClose(uint32_t fileHandle);
  static bool FileSeek(uint32_t fileHandle, FileSeekMode fileSeekMode,
      uint32_t offset);
  ...
  static bool FileCopy(const std::string sourcePath,
      const std::string destPath);
  static bool FileMove(const std::string sourcePath,
      const std::string destPath);
  ...
  static std::string CombinePath(const std::string first,
