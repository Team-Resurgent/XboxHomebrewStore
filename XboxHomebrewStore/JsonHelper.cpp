...
#include "String.h"
JSON_Value *JsonHelper::GetObjectMember(const JSON_Object *obj,
    const char *name) {
  if (!obj || !name) {
    return nullptr;
  }
...
int32_t JsonHelper::ToInt(const JSON_Value *v) {
  if (!v) {
    return 0;
  }
  return (int32_t)json_value_get_number(v);
}
...
JSON_Value *JsonHelper::NumberValue(int32_t n) {
...
bool JsonHelper::ObjectAdd(JSON_Object *obj, const char *keyName, size_t keyLen,
    JSON_Value *value) {
  (void)keyLen;
