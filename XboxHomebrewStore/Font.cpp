...
void MeasureInternal(BitmapFont *bitmapFont, const std::string message,
    float *outWidth) {
  float width = 0;
  ...
  while (*p) {
    ...
    p = cursor;
    std::map<uint32_t, Rect>::const_iterator it =
        bitmapFont->charmap.find(unicode);
...
void MeasureTextWrappedInternal(BitmapFont *font, const std::string message,
    float maxWidth, float *outWidth,
    float *outHeight) {
  float lineWidth = 0;
  ...
  while (*p) {
    ...
    float charWidth =
        (float)rect.width + (lineWidth > 0 ? (float)font->spacing : 0);

    if (unicode == ' ') {
      const char *nextWordStart = p;
      float nextWordW = MeasureWordWidth(font, nextWordStart, &nextWordStart);
      if (lineWidth > 0 &&
          lineWidth + (float)font->spacing + (float)rect.width + nextWordW >
              maxWidth) {
        if (lineWidth > maxLineWidth) {
          maxLineWidth = lineWidth;
        }
      ...
      lineWidth +=
          (lineWidth > 0 ? (float)font->spacing : 0) + (float)rect.width;
    } else {
...
void Font::Init() {
  if (Drawing::LoadFont("D:\\Media\\Fonts\\Font.sfn", &mMainFontContext)) {
    Drawing::TryGenerateBitmapFont(&mMainFontContext, "FreeSans",
        SSFN_STYLE_REGULAR, 16, 16, 0, 256,
        &mNormalMainFont);
    Drawing::TryGenerateBitmapFont(&mMainFontContext, "FreeSans",
        SSFN_STYLE_REGULAR, 24, 24, 0, 512,
        &mLargeMainFont);
  }
}

void Font::MeasureText(const FontType font, const std::string message,
    float *outWidth) {
  MeasureInternal(GetBitmapFont(font), message, outWidth);
}

void Font::MeasureTextWrapped(const FontType font, const std::string message,
    float maxWidth, float *outWidth,
    float *outHeight) {
  MeasureTextWrappedInternal(GetBitmapFont(font), message, maxWidth, outWidth,
      outHeight);
}

std::string Font::TruncateText(const FontType font, const std::string message,
    float maxWidth) {
  ...
  while (*p) {
    ...
    int32_t charW =
        it->second.width + (truncated.empty() ? 0 : bitmapFont->spacing);
...
void Font::DrawText(const FontType font, const std::string message,
    uint32_t color, float x, float y) {
  Drawing::DrawFont(GetBitmapFont(font), message, color, x, y);
}

void Font::DrawTextWrapped(const FontType font, const std::string message,
    uint32_t color, float x, float y, float maxWidth) {
  Drawing::DrawFontWrapped(GetBitmapFont(font), message, color, x, y, maxWidth);
}

void Font::DrawTextScrolling(const FontType font, const std::string message,
    uint32_t color, float x, float y, float maxWidth,
