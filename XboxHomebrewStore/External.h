...
NTSTATUS WINAPI NtOpenFile(OUT PHANDLE pHandle, IN ACCESS_MASK DesiredAccess,
...
NTSTATUS WINAPI NtReadFile(HANDLE Handle, HANDLE Event, PVOID pApcRoutine,
    PVOID pApcContext, PVOID pIoStatusBlock,
    PVOID pBuffer, ULONG Length,
    PLARGE_INTEGER pByteOffset);

NTSTATUS WINAPI NtSetSystemTime(PLARGE_INTEGER SystemTime,
    PLARGE_INTEGER PreviousTime);

extern STRING *XeImageFileName;
VOID WINAPI HalReturnToFirmware(ULONG value);
NTSTATUS WINAPI HalWriteSMBusValue(UCHAR devddress, UCHAR offset, UCHAR writedw,
    DWORD data);
NTSTATUS WINAPI HalReadSMBusValue(UCHAR devddress, UCHAR offset, UCHAR readdw,
    DWORD *pdata);
...
NTSTATUS WINAPI MU_CreateDeviceObject(uint32_t port, uint32_t slot,
    STRING *deviceName);
