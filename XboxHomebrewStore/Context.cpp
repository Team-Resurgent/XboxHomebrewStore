...
void Context::SetD3dDevice(D3DDevice *d3dDevice) { mD3dDevice = d3dDevice; }

D3DDevice *Context::GetD3dDevice() { return mD3dDevice; }

void Context::SetSceneManager(SceneManager *pMgr) { s_pSceneManager = pMgr; }
...
void Context::SetSafeAreaInsets(float left, float top, float right,
    float bottom) {
...
SceneManager *Context::GetSceneManager() { return s_pSceneManager; }

float Context::GetScreenWidth() { return (float)s_logicalWidth; }

float Context::GetScreenHeight() { return (float)s_logicalHeight; }

float Context::GetActualScreenWidth() { return (float)s_actualWidth; }

float Context::GetActualScreenHeight() { return (float)s_actualHeight; }

float Context::GetScaleX() {
  return s_logicalWidth > 0 ? (float)s_actualWidth / (float)s_logicalWidth
                            : 1.0f;
}

float Context::GetScaleY() {
  return s_logicalHeight > 0 ? (float)s_actualHeight / (float)s_logicalHeight
                             : 1.0f;
}

float Context::GetSafeAreaLeft() { return s_safeLeft; }

float Context::GetSafeAreaTop() { return s_safeTop; }

float Context::GetSafeAreaRight() { return s_safeRight; }

float Context::GetSafeAreaBottom() { return s_safeBottom; }

float Context::GetSafeAreaX() { return s_safeLeft; }

float Context::GetSafeAreaY() { return s_safeTop; }
...
int32_t Context::GetGridRows() {
  float gridHeight =
      GetScreenHeight() - (ASSET_HEADER_HEIGHT + ASSET_FOOTER_HEIGHT);
  return (int32_t)((gridHeight + CARD_GAP) / (ASSET_CARD_HEIGHT + CARD_GAP));
}

int32_t Context::GetGridCells() { return GetGridCols() * GetGridRows(); }